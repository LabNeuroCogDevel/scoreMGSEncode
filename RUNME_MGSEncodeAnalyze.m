%%%%%%
% This is a copy of David M.'s function: MGSEncodeAnalyze.m
% made to work for just one eyd file instead of loading a spreadsheet of file locations
% 2012-11-01 WF
%
% call like: result = RUNME_MGSEncodeAnalyze('/mnt/B/bea_res/Data/Tasks/MGSEncode/Basic/10180/20070113/Raw/EyeData/10180_2039_mgsencode2.eyd')
%
% Function takes a path to an eyd file
% returns "results"
%
% N.B. first 4 in david's original are: lunaid, age, sexid and visit 
%  these are all set arbitrarly to 1 and added at the end of this script
%  so next steps of ProcessMGSEncodeResults and then RescoreMGSEncode
%  will run (visit is checked 1:10, run 1:3)
%    checkout mgsEncodeData, correctArry, errorArray
%
% 1     runNumber,       
% 2     trialNumber,     
% 3     startCode,       
...
% 12    trialStartTime   
% 13    targetCode,     
% 14-16 fileCreationTime (from e.g. 5 12 53 PM)
% 
%%%%
function [results] = RUNME_MGSEncodeAnalyze(eydpath)
global fileCreationTime
...
try
    ...
    [pname,fname, ext] = fileparts(eydpath); % .eyd is dropped if not asked for
    pname=[pname '/'];
    fname=[fname ext];
    ...
    % Remove any duplicate XDAT codes000
...
if success
    ...
    if hasCodes && size(index,1) > 1
        ...
        results = [ ones(size(results,1),4), results];
